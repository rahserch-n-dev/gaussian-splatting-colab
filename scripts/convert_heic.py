...
def convert_heic_to_jpg(
    ...
    for i, heic_file in enumerate(heic_files, 1):
        ...
        if jpg_file.exists():
            print(
                f"[{i}/{len(heic_files)}] Skipping {heic_file.name} (already converted)"
            )
            continue
        ...
        try:
            ...
            print(
                f"[{i}/{len(heic_files)}] Converted {heic_file.name} -> {jpg_file.name}"
            )
...
def main():
    ...
    parser.add_argument(
        "--input_dir", "-i", required=True, help="Directory containing HEIC files"
    )
    parser.add_argument(
        "--output_dir", "-o", help="Output directory (default: same as input)"
    )
    parser.add_argument(
        "--quality", "-q", type=int, default=95, help="JPG quality 1-100 (default: 95)"
    )

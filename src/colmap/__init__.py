...
def run_colmap(
    ...
    if not os.path.isfile(convert_script):
        ...
        convert_script = os.path.join(
            project_root, "external", "gaussian-splatting", "convert.py"
        )

    if not os.path.isfile(convert_script):
        raise FileNotFoundError(
            f"COLMAP convert script not found. Tried:\n"
    ...
    if os.path.exists(input_path) and input_path != input_dir:
        if not os.path.exists(input_dir):
            ...
            try:
                ...
                print("Symlink failed, copying images instead...")
    ...
    if rc != 0 and need_retry and use_gpu:
        print(
            "Detected OpenGL / GPU context issue in COLMAP. "
            "Retrying feature extraction with CPU-only mode (--no_gpu). "
            f"See logs at: {colmap_log}"
        )
